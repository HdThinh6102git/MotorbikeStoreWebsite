...
@WebServlet("/filterBranch")
public class filterBranchController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String idBranchStr = request.getParameter("id");
		int idBranch = 0;
		if(idBranchStr != null) {
			idBranch = Integer.parseInt(idBranchStr);
		}
		...
		if(idBranch == 0) {
			...
			list = productDetailService.filterBranch("honda");
		}else if(idBranch == 2) {
			list = productDetailService.filterBranch("yamaha");
		}else if(idBranch == 3) {
			list = productDetailService.filterBranch("suzuki");
		}else if(idBranch == 4) {
			list = productDetailService.filterBranch("sym");
		}
