...
@WebServlet("/filterCC")
public class filterCCController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String idSpeedStr = request.getParameter("id");
		int idSpeed = 0;
		if(idSpeedStr != null) {
			idSpeed = Integer.parseInt(idSpeedStr);
		}
		...
		if(idSpeed == 0) {
			...
			list = productDetailService.filterCC(50, 110);
		}else if(idSpeed == 2) {
			list = productDetailService.filterCC(110, 125);
		}else if(idSpeed == 3) {
			list = productDetailService.filterCC(125, 150);
		}
