...
public class filterPriceController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String idPriceStr = request.getParameter("id");
		int idPrice = 0;
		if(idPriceStr != null) {
			idPrice = Integer.parseInt(idPriceStr);
		}
		...
		if(idPrice == 0) {
			...
			list = productDetailService.filterPriceAround(100, 500);
		}else if(idPrice == 2) {
			list = productDetailService.filterPriceAround(500, 1000);
		}else if(idPrice == 3) {
			list = productDetailService.filterPriceAround(1000, 3000);
		}
