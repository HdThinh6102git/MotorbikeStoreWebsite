...
public class filterColorController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String idColorStr = request.getParameter("id");
		int idColor = 0;
		if(idColorStr != null) {
			idColor = Integer.parseInt(idColorStr);
		}
		...
		if(idColor == 0) {
			...
			list = productDetailService.filterColor("đỏ");
		}else if(idColor == 2) {
			list = productDetailService.filterColor("xanh");
		}else if(idColor == 3) {
			list = productDetailService.filterColor("đen");
		}
